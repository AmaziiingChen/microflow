...
try:
    ...
    from src.spiders.music_spider import MusicSpider
    ...
    for spider in spiders:
        ...
        for section_name, entry_path in sections.items():
            ...
            with spider:
                ...
                logger.info(
                    f"     ✅ 成功解析翻页规律，该板块实际共有 {len(all_page_urls)} 页。"
                )
                ...
                for i, item in enumerate(collected_articles, 1):
                    ...
                    exact_time = "获取失败"  # 🌟 修复：在块外初始化，防止 NameError

                    if detail:
                        # 🌟 1. 提取底层精确时间
                        exact_time = (
                            detail.get("exact_time") or "未提供精确到分秒的时间"
                        )
                        ...
                        body_preview = (
                            body[:35].replace("\n", " ") + "..."
                            if body
                            else "正文为空(可能为纯图片或容器异常)"
                        )

                        atts = detail.get("attachments", [])
                        if atts:
                            attachments_info = (
                                f"发现 {len(atts)} 个附件 -> [{atts[0]['name']}]"
                            )
