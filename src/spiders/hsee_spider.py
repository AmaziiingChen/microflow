...
class HseeSpider(BaseSpider):
    ...
    def _fetch_section_list(self, entry_url: str, section: str, page_num: int) -> List[ArticleData]:
        ...
        # HSEE 特殊 DOM 结构：<div class="right n_list"> 内的 <ul> 中
        # 列表项为 <li class="cleafix">（注意拼写错误）
        items = soup.select('.n_list ul li.cleafix')

        if not items:
            # 备用选择器：直接搜索带 cleafix 类的 li
    ...
    def _calculate_page_url(self, entry_url: str, page_num: int) -> str:
        ...
        # 示例：MAX_PAGE=17 时
        #   - page_num=2 -> 17-2+2 = 17
        #   - page_num=3 -> 17-3+2 = 16
    ...
    def _parse_list_item(self, item: Tag, section: str) -> Optional[ArticleData]:
        ...
        HSEE 列表结构（li.cleafix）：
        - 标题：<a> 标签，优先使用 title 属性，        - 链接：<a> 标签的 href
        - 日期：兄弟节点 <i> 标签的文本

        Args:
            item: BeautifulSoup Tag 元素（li.cleafix）
            section: 板块名称
