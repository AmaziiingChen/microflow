...
class IcocSpider(BaseSpider):
    ...
    def _fetch_section_list(self, entry_url: str, section: str, page_num: int) -> List[ArticleData]:
        ...
        # 查找文章列表容器下的所有 li
        # 尝试多种容器选择器
        containers = soup.find_all('ul', class_='list-gl')
        if not containers:
            containers = soup.find_all('ul', class_='news-list')
        if not containers:
            containers = soup.find_all('ul', class_='list_pic')
    ...
    def _parse_list_item(self, li, section: str) -> Optional[ArticleData]:
        ...
        结构1：<li><a href="..."><div class="b_t">标题</div><div class="date">2026-03-04</div></a></li>
        结构2：<li><a href="..."><h3>标题</h3><p>日期</p></a></li>
    ...
    def _fetch_campus_detail(self, url: str) -> Optional[ArticleData]:
        ...
        # 提取标题：.content_t h3
        title = ""
        title_tag = soup.select_one('.content_t h3')
        if title_tag:
        ...
        body_html = ""
        body_text = ""
        content_div = soup.find('div', class_='content_m')
        ...
        # 提取精确时间：.content_t p（如『发布时间：2026-03-04』）
    ...
    def _extract_exact_time(self, soup: BeautifulSoup) -> str:
        ...
        Returns:
            精确时间字符串
        """
        # 优先从 .content_t p 中提取（如『发布时间：2026-03-04』）
        content_t = soup.find('div', class_='content_t')
        if content_t:
            p_tag = content_t.find('p')
            if p_tag:
