...
import re  # 必须加上这行
def main():
    url = "https://nmne.sztu.edu.cn/info/1073/4372.htm"
    ...
    if not core_container:
        print(json.dumps({"error": "核心内容容器未找到", "url": url}, ensure_ascii=False))
        return
   ...
   # --- 附件提取模块（防污染版：相对 DOM 遍历） ---
    ...
    for a in attach_tags:
        ...
        is_keyword_attach = any(keyword in text.lower() for keyword in ['附件', '下载', '点击查看']) or href.lower().endswith(('.pdf', '.doc', '.docx', '.xls', '.xlsx', '.zip', '.rar'))
    ...
    if meta_pub_date and meta_pub_date.get('content'):
        ...
        if article_wrapper:
            ...
            datetime_match = re.search(r'(20\d{2}[-/年]\d{1,2}[-/月]\d{1,2}(?:\s+\d{1,2}:\d{1,2}(?::\d{1,2})?)?)', header_text)
            
            if datetime_match:
                raw_date = datetime_match.group(1)
                publish_time = raw_date.replace('年', '-').replace('月', '-').replace('/', '-')
